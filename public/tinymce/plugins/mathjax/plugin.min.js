tinymce.PluginManager.add("mathjax", function (editor) {
  ...
  function removeMathJaxOutput(el) {
    // Lấy lại LaTeX gốc từ data-latex
    const latex = decodeURIComponent(el.getAttribute("data-latex") || "");
    // Reset innerHTML về LaTeX gốc (đã wrap đúng kiểu)
  ...
  function insertMath(latex, mode) {
    ...
    if (mode === "block") {
      const mathBlock =
        ...
        "</div>";
      editor.insertContent(mathBlock);
      setTimeout(() => {
        const body = editor.getBody();
        const blocks = body.querySelectorAll(
        ...
        blocks.forEach((block) => {
          ...
          while (
            ...
            (parent.tagName === "P" ||
              parent.tagName === "SPAN" ||
              (parent.classList && parent.classList.contains("math-tex-block")))
          ) {
            parent.after(block);
            if (
